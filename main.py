        ...
        for producto in productos:
            if producto.cod_art not in productos_omitir:
                ...
                datos.append({
                    ...
                    'Estado': estado,
                    'Demanda_Media': round(producto_final.demanda_media, 2) if producto_final.demanda_media != 'NO VALIDO' else 999999,
                    ...
                    'Cobertura_Inicial': round(producto_final.cobertura_inicial, 2) if producto_final.cobertura_inicial != 'NO VALIDO' else 999999,
                    'Cobertura_Final': round(cobertura_final, 2) if cobertura_final != 'NO VALIDO' else 999999,
                    'Stock_final': round(cobertura_final*producto.demanda_media, 2) if cobertura_final != 'NO VALIDO' else 999999,
                    'Cajas Hora' : round(producto.cajas_hora),
                    'Cobertura_Final_Est': round(producto_final.cobertura_final_est, 2) if producto_final.cobertura_final_est != 'NO VALIDO' else 999999,
                    'Desviación': round(cobertura_final - dias_cobertura_base, 0) if cobertura_final != 'NO VALIDO' else 999999,
                })
